...
export function SpeechToTextPlugin() {
  ...
  useEffect(() => {
    ...
    // we show the loader briefly until the client determines support.
    setStatus(SUPPORT_SPEECH_RECOGNITION ? "supported" : "unsupported");
  }, []);

  if (status === "checking") {
    return (
      <Button size="sm" variant="ghost" disabled className="p-2">
