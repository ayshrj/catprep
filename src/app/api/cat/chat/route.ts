...
const TOOLING_INSTRUCTIONS = `You have tool access.\n\nRules:\n- Always call the tool \`cat_intake\` first.\n- Use \`cat_extract_mock_scores\` when the user provides mock scores.\n- Use \`cat_detect_topic\` when the user asks about a topic/question and you need section/tag hints.\n- After tool results, follow them strictly.\n- Answer the user’s question directly first. Only ask onboarding questions when the user explicitly asked for a plan AND the tool says info is missing.\n- If \`cat_intake.responseMode\` is \`onboarding_questions\`, keep it compact: ask only the provided \`quickQuestions\` (no long plan / no exam pattern dump).\n- Otherwise, follow the \`PROMPT_CAT\` output format, but keep greetings and one-liners short.\n- Do not dump large tables unless the user asks.\n`
...
async function getUserSettings(userId: string) {
  ...
  const apiKey =
    typeof data.openRouterApiKey === "string" ? data.openRouterApiKey.trim() : ""
  const model =
...
async function generateCatCoachReply({
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    for (const toolCall of normalizedToolCalls ?? []) {
      ...
      history = [
        ...history,
        { role: "tool", tool_call_id: toolCall.id, content: toolSafeJson(result) },
      ]
    }
  }
