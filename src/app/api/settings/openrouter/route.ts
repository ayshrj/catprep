...
export async function GET() {
  ...
  if (!db) {
    ...
    const key = typeof record?.openRouterApiKey === "string" ? record.openRouterApiKey : "";
  ...
  const key = typeof data.openRouterApiKey === "string" ? data.openRouterApiKey : "";
...
export async function DELETE() {
  ...
  await db
    ...
    .collection("private")
    .doc("settings")
    .set({ openRouterApiKey: null, openRouterModel: null, updatedAt: now }, { merge: true });
...
export async function PATCH(request: Request) {
  ...
  if (!model) {
    return NextResponse.json({ error: "OpenRouter model is required." }, { status: 400 });
  }
